...
$logo_f = $d->rawQueryOne("select id, photo from #_photo where type = ? and act = ? and hienthi > 0 limit 0,1",array('logo-f','photo_static'));
$banner = $d->rawQueryOne("select photo from #_photo where type = ? and act = ? and hienthi > 0 limit 0,1",array('banner','photo_static'));
$banner_po = $d->rawQueryOne("select photo from #_photo where type = ? and act = ? and hienthi > 0 limit 0,1",array('banner-pop','photo_static'));
$b_c = $d->rawQueryOne("select photo from #_photo where type = ? and act = ? and hienthi > 0 limit 0,1",array('banner-lh','photo_static'));
...
/* Footer */
$footer = $d->rawQueryOne("select ten$lang as ten, noidung$lang as noidung, mota$lang as mota from #_static where type = ? limit 0,1",array('footer'));
$mxh = $d->rawQuery("select ten$lang as ten, photo, link from #_photo where type = ? and hienthi > 0 order by stt,id desc limit 0,4",array('mxh'));
...
/* Phone */
$phone = $d->rawQueryOne("select * from #_phone where hienthi > 0");
$countphone = explode('-',$phone['sodienthoai']);
$optionsphone = (isset($phone['options']) && $phone['options'] != '') ? json_decode($phone['options'],true) : null;
...
$ttlistmenu = $d->rawQuery("select ten$lang as ten, tenkhongdauvi, tenkhongdauen, id from #_news_list where type = ? and hienthi > 0 order by stt,id desc",array('du-an'));
$tclistmenu = $d->rawQuery("select ten$lang as ten, tenkhongdauvi, tenkhongdauen, id from #_news where type = ? and hienthi > 0 order by stt,id desc",array('dich-vu-thi-cong'));
$tklistmenu = $d->rawQuery("select ten$lang as ten, tenkhongdauvi, tenkhongdauen, id from #_news_list where type = ? and hienthi > 0 order by stt,id desc",array('dich-vu-thiet-ke'));
...
if(isset($_POST['submit-newsletter']))
{
    ...
    if(($scoreCaptcha >= 0.5 && $actionCaptcha == 'Newsletter') || $testCaptcha == true)
    {
        ...
        $data['congtrinh'] = (isset($_REQUEST['congtrinh-newsletter']) && $_REQUEST['congtrinh-newsletter'] != '') ? htmlspecialchars($_REQUEST['congtrinh-newsletter']) : '';
        $data['phongcach'] = (isset($_REQUEST['phongcach-newsletter']) && $_REQUEST['phongcach-newsletter'] != '') ? htmlspecialchars($_REQUEST['phongcach-newsletter']) : '';
