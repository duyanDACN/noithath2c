...
$slider = $d->rawQuery("select ten$lang as ten, mota$lang as mota, photo, link, link2 from #_photo where type = ? and hienthi > 0 order by stt,id desc",array('slide'));

$gioithieu = $d->rawQueryOne("select id, type, ten$lang as ten, mota$lang as mota, photo, photo2, ngaytao, ngaysua from #_static where type = ? limit 0,1",array('gioi-thieu'));

$giaiphap = $d->rawQuery("select ten$lang as ten, mota$lang as mota, photo, link from #_photo where type = ? and hienthi > 0 order by stt,id desc limit 0,4",array('giaiphap'));

$s_gp = $d->rawQueryOne("select id, type, ten$lang as ten, mota$lang as mota, link, photo, ngaytao, ngaysua from #_static where type = ? limit 0,1",array('s-giaiphap'));
...
$duan = $d->rawQuery("select ten$lang as ten, tenkhongdauvi, tenkhongdauen, text$lang as text, ngaytao, id, photo from #_news where type = ? and noibat > 0 and hienthi > 0 order by stt,id desc",array('du-an'));

$tieuchi = $d->rawQuery("select ten$lang as ten, tenkhongdauvi, tenkhongdauen, text$lang as text, ngaytao, id, photo from #_news where type = ? and noibat > 0 and hienthi > 0 order by stt,id desc",array('tieu-chi'));
