<?php
	...
	function save_static()
	{
		...
		if(isset($_FILES['file2']))
		{
			...
			if($photo2 = $func->uploadImage("file2", $config['static'][$type]['img_type'],UPLOAD_NEWS,$file_name))
			{
				$data['photo2'] = $photo2;
		...
		if(isset($_FILES['file3']))
		{
			...
			if($photo3 = $func->uploadImage("file3", $config['static'][$type]['img_type'],UPLOAD_NEWS,$file_name))
			{
				$data['photo3'] = $photo3;
				$row = $d->rawQueryOne("select id, photo3 from #_static where type = ? limit 0,1",array($type));
				if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_NEWS.$row['photo3']);
			}
		}
