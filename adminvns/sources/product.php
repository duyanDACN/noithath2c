...
function save_item()
{
	...
	if($data)
	{
		...
		{
			$dataSize = (isset($_POST['dataSize'])) ? $_POST['dataSize'] : null;
			$arrayidSize = array();
			foreach ($dataSize as $q => $w) {
				if($w['sizeProduct'] > 0 && $w['sizeProduct'] != ''){
					if (!in_array($w['sizeProduct'], $arrayidSize)) {
						array_push($arrayidSize, $w['sizeProduct']);
					}
				}
			}
			$data['id_size'] = implode(",", $arrayidSize);
		}		
	...
	if($id && $act!='save_copy')
	{
		...
		{
			$dataSizeInsert2['hienthi'] = 0;
			$d->where('id_product',$id);
			$d->update('product_price',$dataSizeInsert2);
			/* SIZE */
			foreach ($dataSize as $i => $row) { 
				if($i > 0) {
					...
					$checkSize = $d->rawQueryOne("select id, id_size, photo from #_product_price where id_size = ? and id_product = ? and type = ?",array($sizeProduct,$id,$type));
					$checkSTT = $d->rawQueryOne("select stt from #_product_price where id_size = ? and id_product = ? and type = ? order by stt desc",array($sizeProduct,$id,$type));

					$dataSizeInsert['photo'] = $checkSize['photo'];

					if(!empty($checkSize['id'])){

						$dataSizeInsert['gia'] = (isset($gia) && $gia != '') ? str_replace(",","",$gia) : 0;
						$dataSizeInsert['giamoi'] = (isset($giamoi) && $giamoi != '') ? str_replace(",","",$giamoi) : 0;
						$dataSizeInsert['stt'] = (int)$row['stt'];
						$dataSizeInsert['hienthi'] = 1;
						$dataSizeInsert['ngaysua'] = time();
						if(isset($_FILES["file-zone-size-".$i]))
						{
							$file_name = $func->uploadName($_FILES["file-zone-size-".$i]["name"]);

							if($photo = $func->uploadImage("file-zone-size-".$i, $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file_name))
							{
								$dataSizeInsert['photo'] = $photo;
								$row = $d->rawQueryOne("select id, photo from #_product_price where id_product = ? and id_size = ? and type = ? limit 0,1",array($idProduct,$sizeProduct,$type));
								if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_PRODUCT.$row['photo']);
							}
						}

						$d->where('id',$checkSize['id']);
						$d->update('product_price',$dataSizeInsert);


					}else{
						$dataSizeInsert['id_product'] = $idProduct;
						$dataSizeInsert['id_size'] = $sizeProduct;
						$dataSizeInsert['gia'] = (isset($gia) && $gia != '') ? str_replace(",","",$gia) : 0;
						$dataSizeInsert['giamoi'] = (isset($giamoi) && $giamoi != '') ? str_replace(",","",$giamoi) : 0;
						$dataSizeInsert['type'] = $type;
						$dataSizeInsert['ngaytao'] = time();
						$dataSizeInsert['stt'] = (int)$row['stt'];
						$dataSizeInsert['hienthi'] = 1;
						...
						{
							$file_name = $func->uploadName($_FILES["file-zone-size-".$i]["name"]);
							if($photo = $func->uploadImage("file-zone-size-".$i, $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file_name.$i))
							{
								$dataSizeInsert['photo'] = $photo;
							}
						}
						$d->insert('product_price',$dataSizeInsert);
					}
				} 
			}
		...
		if($d->insert('product',$data))
		{
			...
			foreach ($dataSize as $i => $row) {
				...
				// Lấy giá trị từng cột
					...
					// Kiểm tra size có tồn tại
					$checkSize = $d->rawQueryOne("select id, id_size, photo from #_product_price where id_size = ? and id_product = ? and type = ?",array($sizeProduct,$idProduct,$type));
