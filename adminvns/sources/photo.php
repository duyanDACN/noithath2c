	...
	function save_static()
	{
		...
		if($id)
		{
			...
			if(isset($_FILES['video-file']))
			{
				...
				if($taptin = $func->uploadImage("video-file", $config['photo']['photo_static'][$type]['file_type'],UPLOAD_FILE,$file_name))
				{
					$data['taptin'] = $taptin;
		...
		else
		{
			...
			if($numberPhoto && $dataMultiTemp)
			{
				...
				for($i=0;$i<count($dataMultiTemp);$i++)
				{
					...
					if(isset($_FILES['video-file-'.$i]))
						{
							$file_name = $func->uploadName($_FILES['video-file-'.$i]["name"]);
						
							if($taptin = $func->uploadImage("video-file-".$i, $config['photo']['man_photo'][$type]['file_type'],UPLOAD_FILE,$file_name))
							{
								$dataMulti['taptin'] = $taptin;
								$row = $d->rawQuery("select id, taptin from #_photo where type = ? limit 0,1",array($type));
					...
					if(isset($config['photo']['man_photo'][$type]['video-file']) && $config['photo']['man_photo'][$type]['video-file'] == true)
					{
						if(!empty($_FILES['video-file-'.$i]))
						{
							$success_file = 1;
						}
					}

					if($success_photo || $success_video || $success_file )
					{
