				...
				onEnd: function(){
					...
					$.ajax({
						...
						success: function(result)
						{
							...
							const button = document.getElementById('sort-filer');
							button.click();
						}
					})
				},
...
function slugPress()
{
	...
	$(document).ready(function(){
		...
		$('body').on('click','.click-checkbox',function(){
		...
		$(window).scroll(function(){
			var caotop = $(".main-header").height();
			if ($(window).width() <= 768){
				if($(window).scrollTop() > 0) {
					$(".content-header-vns").addClass('scroll-top');
					$(".sidebar").addClass('scroll-top');
				}
				else {
					$(".content-header-vns").removeClass('scroll-top');
					$(".sidebar").removeClass('scroll-top');
				}
			}else{
		...
		$(window).load(function() {
			...
			var act = $("input[name='actS']").val();
			if(id>0 || act=='add'){
				if($(".sidebar-mini").hasClass('sidebar-collapse')){
					$('.sidebar-mini').removeClass('sidebar-collapse').addClass('sidebar-collapse');
				}else{
		...
		$('body').on('click','.check-all-filer', function(){
			...
			{
				$(".my-jFiler-item").removeClass('selected');
			...
			{
				$(".my-jFiler-item").addClass('selected');
		...
		$('body').on('click','.my-jFiler-item',function(){
			var input = $(this).find('input.filer-checkbox').val();
			// Lấy checkbox
			const checkbox = document.getElementById('filer-checkbox-'+input);
			// Toggle dựa trên trạng thái ban đầu
			if (checkbox.checked) {
  		  		checkbox.checked = false; // Bỏ chọn
  		  	} else {
    			checkbox.checked = true; // Chọn
    		}
    	})
		/* Sort filer */
