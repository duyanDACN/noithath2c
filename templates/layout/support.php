<?php 
$bottomPhone = $optionsphone['destop']['bottom'];
$leftPhone = ($optionsphone['destop']['left'] > 0 ) ? $optionsphone['destop']['left'] : 0;
$rightPhone = ($optionsphone['destop']['right'] > 0 ) ? $optionsphone['destop']['right'] : 0;

foreach ($countphone as $q => $w): ?>
    <?php if($optionsphone['destop']['device'] == 'on' && $optionsphone['destop']['bottom'] > 0) {?>
        <a class="btn-phone btn-frame text-decoration-none" href="tel:<?=preg_replace('/[^0-9]/','',$w);?>" style="bottom:<?=($q==0) ? $bottomPhone : $bottomPhone + 80*$q?>px ; left:<?=($leftPhone > 0) ? $leftPhone."px;" :  "unset"?>; right:<?=($rightPhone > 0) ? $rightPhone."px;" :  "unset"?>">
            <div class="animated infinite zoomIn kenit-alo-circle" style="border-color:#<?=$optionsphone['background-text']?>"></div>
            <div class="animated infinite pulse kenit-alo-circle-fill" style="background:#<?=$optionsphone['background-text']?>"></div>
            <span style="background: #<?=$optionsphone['background-text']?>"><i aria-hidden="true" style="background:#<?=$optionsphone['background']?>; color: #<?=$optionsphone['color']?>"><img src="assets/images/phone.png" alt="Hotline"></i></span>
        </a>
    <?php }else {?>
        <a class="btn-phone btn-frame text-decoration-none" href="tel:<?=preg_replace('/[^0-9]/','',$optsetting['hotline']);?>" >
            <div class="animated infinite zoomIn kenit-alo-circle"></div>
            <div class="animated infinite pulse kenit-alo-circle-fill"></div>
            <span style="background: #<?=$optionsphone['background-text']?>"><i><img src="assets/images/phone.png" alt="Hotline"></i></span>
        </a>
    <?php }?>
<?php endforeach ?>
